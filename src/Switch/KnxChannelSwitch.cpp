...
KnxChannelSwitch::KnxChannelSwitch(std::vector<SwitchBridge *> *switchBridges, uint16_t channelIndex)
    : KnxChannelBase(channelIndex),
      switchBridges(switchBridges)
{
    for (std::vector<SwitchBridge *>::iterator it = switchBridges->begin(); it != switchBridges->end(); ++it)
        (*it)->initialize(this);
}
